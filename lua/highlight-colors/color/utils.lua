local buffer_utils = require("highlight-colors.buffer_utils")
local css_named_colors = require("highlight-colors.named-colors.css")
local tailwind_named_colors = require("highlight-colors.named-colors.tailwind")
local converters = require("highlight-colors.color.converters")
local patterns = require("highlight-colors.color.patterns")
local ansi_named_colors = require("highlight-colors.named-colors.ansi")
...
function M.get_color_value(color, row_offset, custom_colors, enable_short_hex)
	...
	if patterns.is_rgb_color(color) or patterns.is_roblox_color3_rgb_color(color) then
	...
	if patterns.is_roblox_color3_new_color(color) then
...
function M.get_rgb_values(color)
	local rgb_table = {}
	-- First, find the part of the string inside the parentheses to avoid matching numbers outside of it (e.g. the '3' in Color3)
	local values_inside_parentheses = string.match(color, "%((.*)%)")

	-- If we found something inside the parentheses, search for numbers there
	if values_inside_parentheses then
		for color_number in string.gmatch(values_inside_parentheses, "%d+") do
...
---@return number[]
function M.get_color3_new_values(color)
	local rgb_table = {}
	local values_inside_parentheses = string.match(color, "%((.*)%)")

	if values_inside_parentheses then
		for color_number in string.gmatch(values_inside_parentheses, "%d*%.?%d+") do
			-- Convert 0-1 decimal to 0-255 integer and insert into table
			local decimal_value = tonumber(color_number)
			table.insert(rgb_table, math.floor(decimal_value * 255))
		end
	end
