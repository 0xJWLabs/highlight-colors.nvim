...
---@usage create_highlight_name("#FFFFFF") => Returns "highlight-colors-FFFFFF"
---@return string
function M.create_highlight_name(color_value)
	return "highlight-colors-"
		.. string.gsub(color_value, "#", ""):gsub("\\[0-9]*%[", ""):gsub("[!(),%s%.-/%%=:\"'%%[%];#]+", "")
end
...
---@param options {custom_colors: table, virtual_symbol: string, virtual_symbol_prefix: string, virtual_symbol_suffix: string, virtual_symbol_position: 'inline' | 'eol' | 'eow', enable_short_hex: boolean}
